...
export async function getNotificationById(notificationId: string): Promise<NotificationRow | null> {
  const pool = getPool();
  const { rows } = await pool.query(
    `SELECT id, user_id, type, title, message, related_asset_id, read, created_at
...
export async function createNotification(input: {
  ...
  const { rows } = await pool.query(
    ...
    [id, input.userId, input.type, input.title, input.message, input.relatedAssetId ?? null]
  );
