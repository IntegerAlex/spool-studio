...
async function getPresignedUploadUrl(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown R2 presign error';
    console.error('[r2][presign-upload] generation failed', {
      bucket: bucketName,
      key: input.key,
      message,
    });
    throw new Error(`R2 presigned upload URL generation failed for key "${input.key}": ${message}`);
  }
}
...
async function getPresignedDownloadUrl(
  ...
  try {
    ...
    throw new Error(`R2 presigned download URL generation failed for key "${key}": ${message}`);
  }
}
...
async function getFileMetadata(key: string): Promise<R2FileMetadata> {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown R2 metadata error';
