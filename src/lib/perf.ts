          ...
          try {
            ...
            if (result && typeof result.then === 'function') {
              return result.finally(() => {
                ...
                logPerf('[query]', { method: `${prefix}/${String(prop)}`, duration });
              });
            }
            ...
            const duration = Date.now() - start;
            logPerf('[query]', { method: `${prefix}/${String(prop)}`, duration, error: true });
            throw err;
          }
        };
      }
...
export function wrapWithTiming<T extends object>(obj: T, label = 'object'): T {
  if (process.env.PERF_DIAG === '1' || process.env.NEXT_PUBLIC_PERF_DIAG === '1') {
    try {
