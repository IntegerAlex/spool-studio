      ...
      job.fn()
        ...
        .catch((err) => {
          ...
          if (job.retriesLeft > 0) {
            ...
            const base = 1000;
            const backoff = Math.min(30000, base * Math.pow(2, attempt));
            const jitter = 0.5 + Math.random();
            const delay = Math.round(backoff * jitter);
            console.warn('[notification][queue] job-failed; retrying', { key: job.key, delay, err: err instanceof Error ? err.message : String(err) });
            setTimeout(() => {
              this.queue.push(job);
              this.schedule();
            }, delay);
          } else {
            console.error('[notification][queue] job-permanent-failure', { key: job.key, error: err instanceof Error ? err.message : String(err) });
          }
          this.schedule();
        });
    }
