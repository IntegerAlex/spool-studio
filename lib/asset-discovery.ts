...
export type AssetUploadedDateFilter =
  | "all"
  | "today"
  | "last_7_days"
  | "last_30_days"
...
function matchesUploadedDateFilter(
  asset: Asset,
  uploadedDate: AssetUploadedDateFilter,
): boolean {
  switch (uploadedDate) {
...
export function sortAssets(assets: Asset[], sortMode: AssetSortMode): Asset[] {
  ...
  sorted.sort((left, right) => {
    switch (sortMode) {
      case "oldest":
        return (
          getAssetTimestamp(left) - getAssetTimestamp(right) ||
      ...
      case "largest_file":
        return (
          (right.fileSize ?? -1) - (left.fileSize ?? -1) ||
          left.title.localeCompare(right.title)
        )
      case "smallest_file":
        return (
          (left.fileSize ?? Number.POSITIVE_INFINITY) -
            (right.fileSize ?? Number.POSITIVE_INFINITY) ||
          left.title.localeCompare(right.title)
        )
      case "alphabetical":
        return (
          left.title.localeCompare(right.title) ||
          getAssetTimestamp(right) - getAssetTimestamp(left)
        )
      default:
        return (
          getAssetTimestamp(right) - getAssetTimestamp(left) ||
...
function describeFileSizeRange(filters: AssetDiscoveryFilters): string | null {
  ...
  const toMb = (value: number) =>
    `${Math.round((value / (1024 * 1024)) * 10) / 10} MB`
...
export function getDiscoveryEmptyState(
  ...
  if (filters.status !== "all") {
    const details = [
      fileSizeDescriptor,
      dateDescriptor,
      filters.metadataFilter !== "all"
        ? assetMetadataLabels[filters.metadataFilter].toLowerCase()
        : null,
    ]
      .filter(Boolean)
      .join(" • ")
  ...
  const fragments = [
    ...
    filters.quickFilters.length > 0
      ? `${filters.quickFilters.length} quick filter${filters.quickFilters.length === 1 ? "" : "s"}`
      : null,
  ].filter(Boolean)
