...
export interface AssetMetadataExtractionResult {
  updates: Pick<Database['public']['Tables']['content_assets']['Update'],
    | 'mime_type'
    | 'file_size'
    | 'file_extension'
    | 'uploaded_at'
...
function detectMediaType(mimeType: string | null | undefined, fileExtension: string | null): AssetMediaType {
...
function readUInt32(buffer: Uint8Array, offset: number): number {
  return (buffer[offset] * 2 ** 24) + (buffer[offset + 1] * 2 ** 16) + (buffer[offset + 2] * 2 ** 8) + buffer[offset + 3];
}

function parsePng(buffer: Uint8Array): { width: number; height: number } | null {
  if (buffer.length < 24) {
    return null;
  }
...
function parseGif(buffer: Uint8Array): { width: number; height: number } | null {
  if (buffer.length < 10) {
    return null;
  }
...
function parseWebp(buffer: Uint8Array): { width: number; height: number } | null {
  if (buffer.length < 30) {
    return null;
  }
...
function parseJpeg(buffer: Uint8Array): { width: number; height: number } | null {
...
function parseMp4Boxes(
  ...
  result: { durationSeconds: number | null; width: number | null; height: number | null }
): void {
  let offset = start;
...
export async function extractImageMetadata(file: File): Promise<{
  ...
  try {
    ...
    const dimensions = parsePng(buffer) ?? parseJpeg(buffer) ?? parseGif(buffer) ?? parseWebp(buffer);
    if (!dimensions) {
    ...
    partialFailures.push(error instanceof Error ? error.message : 'Unknown image metadata error');
    return {
      mediaWidth: null,
      mediaHeight: null,
      partialFailures,
...
export async function extractVideoMetadata(file: File): Promise<{
  ...
  try {
    ...
    partialFailures.push(error instanceof Error ? error.message : 'Unknown video metadata error');
    return {
      mediaWidth: null,
      mediaHeight: null,
...
export async function extractAssetMetadata(
  ...
  const mediaType = detectMediaType(input.mimeType ?? input.file.type, fileExtension);
