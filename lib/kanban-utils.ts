...
export const statusOrder: KanbanWorkflowColumnId[] = [
  ...kanbanWorkflowColumnIds,
]

export function getNextStatus(currentStatus: AssetStatus): AssetStatus | null {
  const currentIndex = getKanbanWorkflowColumnIndex(
    getKanbanWorkflowColumnId(currentStatus),
  )
  if (currentIndex === -1 || currentIndex === statusOrder.length - 1)
    return null
  return getKanbanWorkflowStatusForColumn(
    kanbanWorkflowColumnIds[currentIndex + 1],
  )
}
