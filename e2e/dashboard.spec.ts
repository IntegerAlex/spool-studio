...
test.describe('Dashboard', () => {
  ...
  test('should have quick action buttons', async ({ page }) => {
    await expect(page.locator('text=New Asset').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should have timeframe selector', async ({ page }) => {
    await expect(page.locator('button:has-text("Weekly")')).toBeVisible({ timeout: 10000 });
  ...
  test('should switch timeframe', async ({ page }) => {
    ...
    await expect(page.locator('text=Weekly Goals')).toBeVisible({ timeout: 5000 });

    await page.click('button:has-text("Monthly")');
    await expect(page.locator('text=Monthly Goals')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('should display top active clients table', async ({ page }) => {
    await expect(page.locator('text=Top Active Clients')).toBeVisible({ timeout: 10000 });
  });

  test('should display reels and posters overview', async ({ page }) => {
    await expect(page.locator('text=Reels Overview')).toBeVisible({ timeout: 10000 });
