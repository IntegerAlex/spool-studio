...
function getActionIcon(action: string): React.ReactNode {
  ...
  if (action.includes("approve") || action.includes("published"))
...
export default function LogsPage() {
  ...
  const actionCounts = useMemo(() => {
    ...
    for (const entry of logs) {
      const key =
        entry.action.includes("login") || entry.action.includes("auth")
          ? "auth"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Complete system activity log with user actions, asset changes, and
            status transitions
          </p>
        </div>
        <Button
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-[11px] font-medium text-zinc-500">Showing</span>
        <span className="rounded-md border border-white/[0.06] bg-white/[0.03] px-2 py-0.5 text-[11px] font-medium text-white">
        ...
        {Object.entries(actionCounts)
          ...
          .map(([key, count]) => (
            <span
              key={key}
              className="rounded-md border border-white/[0.06] bg-white/[0.02] px-2 py-0.5 text-[10px] capitalize text-zinc-400"
            >
              {key.replace(/_/g, " ")}:{" "}
      ...
      <div className="rounded-xl border border-white/[0.06] bg-[var(--color-bg-surface)] overflow-hidden">
        <div className="grid grid-cols-[44px_1fr_140px_100px_140px] gap-3 bg-white/[0.02] border-b border-white/[0.06] px-4 py-2.5">
          <div className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 text-center">
            Event
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500">
            Description
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 hidden sm:block">
            User
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 hidden md:block">
            Entity
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 text-right">
            When
          </div>
        </div>
            ...
            {logs.map((entry, i) => (
              ...
              >
                <div className="flex items-center justify-center">
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full ${getActionBg(entry.action)}`}
                  >
                  ...
                  <p className="text-[11px] text-zinc-500 truncate mt-0.5">
                    {entry.entityName ? (
                      entry.entityType === "asset" ? (
                        ...
                        <Link
                          href={`/dashboard/clients/${entry.entityId}`}
                          className="hover:text-emerald-400 transition-colors"
                        >
                          {entry.entityName}
                        </Link>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06] bg-white/[0.01]">
            <p className="text-[12px] text-zinc-500">
              ...
              <span className="font-medium text-zinc-400">
                {Math.min(total, (currentPage - 1) * ITEMS_PER_PAGE + 1)}
              </span>
              –
              <span className="font-medium text-zinc-400">
                {Math.min(total, currentPage * ITEMS_PER_PAGE)}
              </span>{" "}
              of <span className="font-medium text-zinc-400">{total}</span>
            </p>
            <div className="flex items-center gap-2">
              <button
