...
async function sendPushToSubscription(
  ...
  try {
    ...
    if (!vapidPrivateKey) {
      console.warn('[push-send] VAPID_PRIVATE_KEY not configured, skipping push');
      return false;
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (subscriptions.length === 0) {
      return NextResponse.json({ data: { sent: 0, message: 'No push subscriptions found' } });
    }
    ...
    return NextResponse.json({ data: { sent: sentCount, failed: failedEndpoints.length } });
  } catch (error) {
