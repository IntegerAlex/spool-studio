...
export async function POST(request: Request) {
  try {
    ...
    const user = rows[0];
    const resetToken = await signToken({ sub: user.id, email: user.email, role: 'designer' });
    ...
    if (existing.length > 0) {
      await pool.query('UPDATE password_reset_tokens SET token = $1, expires_at = NOW() + INTERVAL \'1 hour\', used = false WHERE user_id = $2', [resetToken, user.id]);
    } else {
      await pool.query(
    ...
    if (process.env.MAILGUN_API_KEY && process.env.MAILGUN_DOMAIN) {
      try {
        ...
        form.set('text', `Click here to reset your password: ${resetUrl}\n\nThis link expires in 1 hour.`);
        ...
        console.warn('[auth] failed to send reset email via Mailgun, falling back to console', mailErr);
