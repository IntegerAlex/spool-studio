...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!token || !assetId) {
      return NextResponse.json({ error: 'Token and asset id are required' }, { status: 400 });
    }
    ...
    if (assetRows[0].client_id !== portalToken.client_id) {
      return NextResponse.json({ error: 'Asset does not belong to this client' }, { status: 403 });
    }
    ...
    if (decision !== 'approved' && decision !== 'revision_requested') {
      return NextResponse.json({ error: 'Decision must be "approved" or "revision_requested"' }, { status: 400 });
    }
