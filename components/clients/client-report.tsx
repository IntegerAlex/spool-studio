...
export function ClientReport({
    ...
    const fetchReport = async () => {
      ...
      try {
        const res = await fetch(
          buildReportUrl(`/api/clients/${clientId}/report`),
        )
        if (!res.ok) {
  ...
  }, [clientId, reportMode, isCustomRangeValid, buildReportUrl])

  const handleDownloadPDF = async () => {
    setIsDownloading(true)
    try {
      const response = await fetch(
        buildReportUrl(`/api/clients/${clientId}/report/pdf`),
      )
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 rounded-[10px] border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)]">
          <p className="text-[#71717a] text-[13px]">
            Select a start and end date to view the report.
          </p>
        </div>
      ) : report ? (
        <>
          ...
          <Card className="rounded-[10px] border border-[rgba(255,255,255,0.07)] bg-[#161616] p-6 shadow-none">
            <h3 className="text-[12px] font-semibold uppercase tracking-wider text-[#71717a] mb-4">
              Executive Summary
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-1">
                <p className="text-[11px] text-[#71717a] uppercase">
                  Client Name
                </p>
              ...
              <div className="space-y-1">
                ...
                <p className="text-[14px] font-semibold text-white">
                  ...
                  / {report.summary.monthlyTarget}
                </p>
                <p className="text-[12px] text-[#71717a]">
                  {report.summary.targetLabel || "Monthly Target"} — Completion:{" "}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-5">
            ...
            <Card className="col-span-1 sm:col-span-2 rounded-[10px] border border-emerald-500/20 bg-emerald-500/[0.03] p-5 shadow-none flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-emerald-400">
                  Completion Rate
                </p>
                <p className="mt-2 text-[36px] font-bold text-white leading-none">
                  {report.summary.completionRate}%
                </p>
              </div>
              <div className="mt-4 space-y-2">
                <div className="w-full h-2 bg-[rgba(255,255,255,0.08)] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, report.summary.completionRate)}%`,
                    }}
                  />
                </div>
                <p className="text-[11px] text-[#a1a1aa] font-medium">
                  {report.summary.totalDelivered} of{" "}
                  {report.summary.monthlyTarget} Delivered
                </p>
              </div>
            </Card>
            ...
            <Card className="rounded-[10px] border border-[rgba(255,255,255,0.07)] bg-[#161616] p-4 shadow-none flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-[#71717a]">
                  Posters Delivered
                </p>
              ...
              <Card className="rounded-[10px] border border-[rgba(255,255,255,0.07)] bg-[#161616] p-5 shadow-none">
                <div className="flex flex-col gap-1 border-b border-[rgba(255,255,255,0.05)] pb-4 mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[10px] border-[rgba(255,255,255,0.08)] text-[#71717a]"
                    >
                      Recent 5
                    </Badge>
                  </div>
                  <p className="text-[11px] text-[#71717a]">
                    Showing latest 5 deliveries
                  </p>
                </div>
                  ...
                  {report.assets.slice(0, 5).map((asset) => (
                    <div key={asset.id} className="relative pl-6 space-y-1.5">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className="text-[9px] px-1 py-0 border-[rgba(255,255,255,0.1)] capitalize text-[#71717a]"
                        >
                          {asset.type}
                        </Badge>
                      </div>
                        ...
                        {asset.uploadedAt && (
                          <div>
                            ...
                            <p className="mt-0.5 text-white">
                              {formatTimelineDate(asset.uploadedAt)}
                            </p>
                          </div>
                        )}
                        {asset.approvedAt && (
                          <div>
                            ...
                            <p className="mt-0.5 text-white">
                              {formatTimelineDate(asset.approvedAt)}
                            </p>
                          </div>
                        )}
                        {asset.publishedAt && (
                          <div>
                            <p className="uppercase tracking-wide font-medium text-emerald-400">
                              Published
                            </p>
                            <p className="mt-0.5 text-emerald-400 font-semibold">
                              {formatTimelineDate(asset.publishedAt)}
                            </p>
                          </div>
                        )}
                      </div>
