...
export function AssetActivitySection({ assetId }: AssetActivitySectionProps) {
  ...
  const loadActivity = async () => {
    setIsLoading(true);
    try {
      const payload = await activityApi.getByAssetId(assetId, { limit: ACTIVITY_LIMIT });
  ...
  return (
    <Card className="border border-[rgba(255,255,255,0.08)] bg-[#161616] p-5">
      ...
      >
        {isLoading ? (
          <div className="py-4 text-[12px] text-[#71717a]">Loading activity...</div>
        ) : activity.length === 0 ? (
          <div className="py-6 text-[12px] text-[#71717a]">No recent activity.</div>
        ) : (
          <div className="space-y-3">
            {activity.map((entry) => {
              const actor = entry.userId ? userMap.get(entry.userId)?.name ?? entry.userId : 'System';
              return (
                <div key={entry.id} className="rounded-[10px] border border-[rgba(255,255,255,0.06)] bg-[#0f0f0f] p-3">
