...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn(
        'bg-[var(--surface-card)] text-card-foreground flex min-w-0 max-w-full flex-col gap-6 rounded-[12px] border border-[rgba(255,255,255,0.07)] py-6 shadow-none',
        className,
      )}
      {...props}
    />
...
function CardAction({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'col-start-2 row-span-2 row-start-1 self-start justify-self-end min-w-0',
        className,
      )}
      {...props}
    />
